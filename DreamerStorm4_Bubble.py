...
class DreamerV3Config:
    ...
    gamma: float = 0.997  # DreamerV3 paper value (was 0.985)
    ...
    # STORM stochasticity parameters (NEW)
...
class STORMWorldModel(nn.Module):
    ...
    def imagine_trajectory(self, initial_state: torch.Tensor, actor, horizon: int, 
        ...
        for _ in range(horizon):
            ...
            with torch.no_grad():
                action_dist = actor(trajectory['states'][-1], hidden)
...
class DreamerV3Bubble(UniversalBubble):
    ...
    ACTION_PARAMS = {
        Actions.ACTION_TYPE_CODE_UPDATE.name: ["intensity"],  # 0.0-1.0
        Actions.ACTION_TYPE_SELF_QUESTION.name: ["complexity"],  # 0.0-1.0  
        Actions.ACTION_TYPE_SPAWN_BUBBLE.name: ["size", "priority"],  # both 0.0-1.0
        Actions.ACTION_TYPE_DESTROY_BUBBLE.name: ["target_id"],  # discrete
        Actions.ACTION_TYPE_NO_OP.name: []
    }
        ...
        with torch.no_grad():
            ...
            for model in self.world_model_ensemble:
                pred_state, _, _, _, _, _, _, _ = model(
                    states.unsqueeze(1) if states.dim() == 2 else states,
                    actions.unsqueeze(1) if actions.dim() == 2 else actions,
                    device=self.device
                )
    ...
    def get_risk_adjusted_action(self, state: torch.Tensor, hidden: Optional[torch.Tensor] = None) -> Dict[str, Any]:
        ...
        self.debug_log(f"{self.object_id}: Computed reward: {total_reward:.4f} "
                      f"(energy: {energy_reward:.4f}, cpu: {cpu_penalty:.4f}, "
                      f"memory: {memory_penalty:.4f}, cache: {cache_bonus:.4f}, "
                      f"stability: {stability_penalty:.4f})")
    ...
    async def train_actor_critic(self):
        ...
        with self.training_error_handler("actor-critic training"):
            with self.memory_efficient_compute():
                ...
                continuations_tensor = torch.stack(all_continuations).squeeze(-1)
                hidden_states_tensor = torch.stack(all_hidden_states)
                values_tensor = torch.stack(all_values).squeeze(-1)
                ...
                for i in range(num_transitions):
                    dist, _ = self.actor(states_tensor[i:i+1], hidden_states_tensor[i:i+1])
                ...
                with torch.no_grad():
                    ...
                    for m in self.world_model_ensemble:
                        ...
                        sampled_states = states_tensor[sample_indices].unsqueeze(1)
                        sampled_actions = F.one_hot(actions_tensor[sample_indices], num_classes=self.config.action_dim).float().unsqueeze(1)
                        next_state, _, _, _, _, _, _, _ = m(sampled_states, sampled_actions, device=self.device)
                ...
                # Recompute critic predictions for gradient flow
                ...
                if self.config.critic_real_data_scale > 0 and len(self.replay_buffer) > 32:
                    ...
                    with torch.no_grad():
                        _, _, _, real_hidden, _, _, _, _ = self.world_model(
                            real_states_tensor.unsqueeze(1), 
                            torch.stack(real_actions).to(self.device).unsqueeze(1), 
                            device=self.device
                        )
    ...
    async def _train_actor_critic_on_real_data(self):
        ...
        with self.training_error_handler("real data actor-critic training"):
            with self.memory_efficient_compute():
                ...
                # Update actor
                self.actor_optimizer.zero_grad()
                action_dist = self.actor(states_tensor, hidden_states)
    ...
    async def _train_world_model_step(self, states_tensor, actions_tensor, rewards_tensor, 
        ...
        # Forward pass
        predicted_next_states, reward_logits, predicted_continuations, hidden_states, dyn_kl_loss, z_t, recon_state, vae_kl_loss = \
            self.world_model(states_tensor, actions_tensor, device=self.device, hidden_state=None)
        ...
        if is_sequence:
            ...
            recon_loss = torch.mean((recon_state - states_tensor.squeeze(1)) ** 2)
        ...
        if is_sequence:
            ...
            state_for_kl = states_tensor.squeeze(1)
        ...
        dyn_kl_loss = torch.max(dyn_kl_loss - self.config.kl_free_bits, torch.tensor(0.0, device=self.device))
        
        # Total loss (weighted per STORM with VAE component)
        total_loss = (recon_loss + reward_loss + continuation_loss + 
                     self.config.beta1 * dyn_kl_loss + 
        ...
        for m, opt in zip(self.world_model_ensemble, self.ensemble_optimizers):
            ...
            pred_next_states, _, _, _, _, _, _ = m(states_tensor, actions_tensor, device=self.device)
        ...
        with torch.no_grad():
            ensemble_preds = []
            for m in self.world_model_ensemble:
                pred, _, _, _, _, _, _, _ = m(states_tensor, actions_tensor, device=self.device)
...
# Runner Script Example - Save as run_dreamerv3_bubble.py
"""
DreamerV3Bubble Runner Script

This script demonstrates how to deploy DreamerV3Bubble for production system management
with all advanced features including multi-agent coordination, distributed training,
real telemetry integration, and safety validation.

Usage:
    # Single agent mode
    python run_dreamerv3_bubble.py --mode single --telemetry prometheus
    
    # Multi-agent mode
    python run_dreamerv3_bubble.py --mode multi --num-agents 3 --agent-id 0
    
    # Distributed training
    torchrun --nproc_per_node=4 run_dreamerv3_bubble.py --mode distributed
    
    # Ablation study
    python run_dreamerv3_bubble.py --mode ablation --num-steps 1000
"""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="DreamerV3Bubble Runner")
    parser.add_argument("--mode", choices=["single", "multi", "distributed", "ablation"], 
                       default="single", help="Execution mode")
    parser.add_argument("--telemetry", choices=["prometheus", "cloudwatch", "simulated"],
                       default="simulated", help="Telemetry source")
    parser.add_argument("--telemetry-endpoint", type=str, default="http://localhost:9090",
                       help="Telemetry endpoint URL")
    parser.add_argument("--num-agents", type=int, default=1, help="Number of agents for multi mode")
    parser.add_argument("--agent-id", type=int, default=0, help="Agent ID for multi mode")
    parser.add_argument("--num-steps", type=int, default=1000, help="Number of training steps")
    parser.add_argument("--safety-mode", choices=["normal", "conservative", "aggressive"],
                       default="normal", help="Safety validation mode")
    ...
    # Create system context (mock for this example)
    context = SystemContext()  # Your actual context implementation
    
    # Configure DreamerV3Bubble
    config = DreamerV3Config(
        # Telemetry configuration
        use_real_data=(args.telemetry != "simulated"),
        telemetry_source=args.telemetry,
        telemetry_endpoint=args.telemetry_endpoint,
        telemetry_auth={"Authorization": "Bearer YOUR_TOKEN"},  # Add real auth
        telemetry_encryption_key="YOUR_ENCRYPTION_KEY" if args.telemetry != "simulated" else None,
        
        # Safety configuration
        enable_action_validation=True,
        safety_mode=args.safety_mode,
        require_action_confirmation=(args.safety_mode == "conservative"),
        
        # Multi-agent configuration
        enable_multi_agent=(args.mode == "multi"),
        agent_id=f"agent_{args.agent_id}",
        ...
        risk_aversion_factor=0.2 if args.safety_mode == "conservative" else 0.1,
        use_stochastic_latents=True,
    ...
    # Create DreamerV3Bubble instance
    dreamer = DreamerV3Bubble(f"dreamerv3_bubble_{args.agent_id}", context, config)
    
    # Run based on mode
    if args.mode == "ablation":
        logger.info("Running ablation study...")
        results = await dreamer.run_ablation_study(num_steps=args.num_steps)
        
        # Save results
        with open(f"{args.checkpoint_dir}/ablation_results.json", "w") as f:
            json.dump(results, f, indent=2)
        
        logger.info("Ablation study complete. Results saved.")
        
    else:
        logger.info(f"Starting DreamerV3Bubble in {args.mode} mode...")
        
        # Training loop
        for step in range(args.num_steps):
            # Ensure initialization
            await dreamer.ensure_initialized()
            
            # Get current state
            if config.use_real_data:
                current_state = await dreamer.fetch_real_telemetry()
                if current_state is None:
                    current_state = dreamer._simulate_state()
            else:
                current_state = dreamer._simulate_state()
            
            # Get action recommendation
            state_vec = dreamer._vectorize_state(current_state)
            if state_vec is not None:
                # Get risk-adjusted action with parameters
                action = dreamer.get_risk_adjusted_action(state_vec.unsqueeze(0))
                
                # Multi-agent coordination
                if config.enable_multi_agent:
                    action = await dreamer.coordinate_multi_agent_action(action)
                
                # Validate action
                validated_action = dreamer.validate_action(action, current_state)
                
                # Log action decision
                if step % 100 == 0:
                    logger.info(f"Step {step}: State summary - CPU: {current_state.get('cpu_percent', 0):.1f}%, "
                               f"Memory: {current_state.get('memory_percent', 0):.1f}%, "
                               f"Action: {validated_action['action_type']}")
                    
                    if validated_action.get("requires_confirmation"):
                        logger.info(f"Action safety score: {validated_action.get('safety_score', 0):.2f}")
            
            # Periodic training
            if step % 10 == 0:
                dreamer.collect_transitions(10)
                await dreamer.train_world_model()
                await dreamer.train_actor_critic()
            
            # Multi-agent state sharing
            if config.enable_multi_agent and step % config.agent_communication_freq == 0:
                await dreamer.share_agent_state()
            
            # Checkpointing
            if step % config.checkpoint_interval == 0:
                validation_loss = await dreamer.compute_validation_loss()
                is_best = validation_loss < dreamer.best_validation_loss if validation_loss else False
                dreamer.save_checkpoint(is_best=is_best)
            
            # Synchronize gradients in distributed mode
            if config.distributed:
                dreamer._sync_gradients()
        ...
        # Export final model
        dreamer.export_model(f"{args.checkpoint_dir}/dreamerv3_bubble_final.pt")
        logger.info("Model exported.")

if __name__ == "__main__":
    # For the artifact, just show the example usage
    pass
...
"""
DreamerV3Bubble Deployment Guide
...
1. Environment Setup:
   ...
   - Required packages: torch, numpy, aiohttp
   
2. Configuration:
   - Set up telemetry endpoints (Prometheus/CloudWatch)
   - Configure SSL certificates for secure communication
   - Set encryption keys in environment variables
   - Configure action validation thresholds based on your system
   
3. Monitoring:
   - Monitor epistemic uncertainty levels
   - Track action rejection rates
   - Watch for training instabilities (NaN/Inf losses)
...
4. Scaling:
   ...
   - Implement load balancing for telemetry queries
   
5. Safety Checklist:
   - ✓ Enable action validation
   - ✓ Set conservative safety mode for production
   - ✓ Configure rate limiting for actions
   - ✓ Enable action confirmation for critical operations
...
2. Hyperparameter Tuning:
   - Adjust imagination_horizon (default: 16)
   - Tune risk_aversion_factor (default: 0.1)
   - Modify ensemble size (default: 3)
   
3. Benchmarking:
   - Compare against baselines using ablation modes
   - Measure sample efficiency vs model-free methods
...
# Initialize DreamerV3Bubble
config = DreamerV3Config(
    use_real_data=False,  # Use simulation for demo
    debug_mode=True,
    imagination_horizon=5,  # Shorter for quick demo
    batch_size=16
)

dreamer = DreamerV3Bubble("demo_agent", context, config)

# Run training loop
for step in range(100):
    await dreamer.ensure_initialized()
    dreamer.collect_transitions(5)
    ...
    if step % 10 == 0:
        ...
        state = dreamer._simulate_state()
        state_vec = dreamer._vectorize_state(state)
        action = dreamer.get_risk_adjusted_action(state_vec.unsqueeze(0))
        print(f"Step {step}: Recommended action: {action['action_type']}")
```

API Reference:
-------------
Key Methods:
- get_risk_adjusted_action(): Get action with uncertainty-based risk adjustment
- validate_action(): Apply safety constraints to proposed actions
- coordinate_multi_agent_action(): Coordinate with other agents
- run_ablation_study(): Compare different configurations
- fetch_real_telemetry(): Get real system metrics
- compute_epistemic_uncertainty(): Measure prediction uncertainty

Configuration Options:
- safety_mode: "normal", "conservative", "aggressive"
- telemetry_source: "prometheus", "cloudwatch", "custom"
- ablation_mode: "none", "no_transformer", "no_distributional", "no_imagination"

Events:
- SYSTEM_STATE_UPDATE: Receive system state updates
- ACTION_TAKEN: Log executed actions
- PREDICT_STATE_QUERY: Request state predictions
- MULTI_AGENT_STATE_SHARE: Share state with other agents

Troubleshooting:
---------------
1. High uncertainty in predictions:
   - Increase training steps
   - Collect more diverse transitions
   - Check for distribution shift in telemetry
   
2. Actions being blocked:
...
4. Poor multi-agent coordination:
